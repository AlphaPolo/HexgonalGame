...
class MyCanvas extends React.Component<
  any,
  { canvasSize: CanvasSizeState; hexSize: number; startHexPoint: Point }
> {
  ...
  canvasGameObjectRender: React.RefObject<
    HTMLCanvasElement
  > = React.createRef();
  ...
  drawMouseHex: OnCurrentHexChange = (pHex, cHex) => {
    ...
    if (cHex && ctx) {
      ...
      this.preDraw
        .map((hex) => this.hexToPixel(hex, this.state.startHexPoint))
  ...
  componentDidMount() {
    ...
    this.demon.position = getCubeNeighbor(
      this.redhood.position,
      HexDirection.NW
    );
  ...
  drawGameObjects() {
    ...
    this.gameObjects.forEach((obj) =>
      obj.draw(ctx, this.state.startHexPoint, this.hexToPixel)
    );
  }
  ...
  drawHexes(ctx: CanvasRenderingContext2D) {
    ...
    for (let r = -rTopSide; r <= rBottomSide; r++) {
      for (let q = -qLeftSide; q <= qRightSide; q++) {
        ...
        if (
          point.x > hexWidth / 2 &&
          point.x < canvasWidth - hexWidth / 2 &&
          point.y > hexHeight / 2 &&
          point.y < canvasHeight - hexHeight / 2
        ) {
  ...
  drawNeighbor(ctx: CanvasRenderingContext2D, centerHex: Hex) {
    ...
    for (let direction = 0; direction < size; direction++) {
      ...
      const neighborPoint = this.hexToPixel(
        neighborHex,
        this.state.startHexPoint
      );
  ...
  hexToPixel(hex: Hex, offset?: Point) {
    let x =
      this.state.hexSize * Math.sqrt(3) * (hex.q + hex.r / 2) +
      (offset?.x || 0);
  ...
  pixelToPointyHex(point: Point) {
    ...
    let q =
      (((point.x - offset.x) * Math.sqrt(3)) / 3 - (point.y - offset.y) / 3) /
      size;
  ...
  render() {
    return (
      <div>
        <canvas
          ref={this.canvasHex}
          height={this.state.canvasSize.canvasHeight}
          width={this.state.canvasSize.canvasWidth}
        />
        <canvas
          ref={this.canvasCoordinates}
          height={this.state.canvasSize.canvasHeight}
          width={this.state.canvasSize.canvasWidth}
        />
        <canvas
          ref={this.canvasGameObjectRender}
          height={this.state.canvasSize.canvasHeight}
          width={this.state.canvasSize.canvasWidth}
        ...
        <CommandList
          ...
          width={300}
          top={0}
          left={this.state.canvasSize.canvasWidth + 2}
        />
        <HexPad
          height={300}
          width={300}
          top={this.state.canvasSize.canvasHeight - 300}
          left={this.state.canvasSize.canvasWidth + 2}
        />
        <DeckDiv
          style={{
            height: "250px",
            width: this.state.canvasSize.canvasWidth + 302,
            left: 0,
            top: this.state.canvasSize.canvasHeight + 2
          }}
        />
