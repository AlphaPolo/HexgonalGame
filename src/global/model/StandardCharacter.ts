...
export class StandardCharacter {
  ...
  moveToDirection(direction: HexDirection, block = false, byAction?: string) {
    let target = getCubeNeighbor(this.position!, direction);
    if (block) {
      this.moveToSamePlace(target, byAction);
    } else {
      this.moveTo(target, byAction);
    }
  }
  ...
  waitForComplete(type: CompleteType, complete: () => void) {
    if (this.subscribes.has(type)) this.subscribes.get(type)?.attach(complete);
    else {
